...
function show_attr_status(theid, goods_id, attr_id_www_wygk_cn)
{
	...
	var www_wygk_cn= theid.parentNode.id.replace('catt_', '');
	document.getElementById('spec_attr_type_'+www_wygk_cn).value=selected_first[0];

	//if(attr_selected()){
	//	return true;
	//}
	if(change_classname(theid,www_wygk_cn)){
		//var formBuy = document.forms['ECS_FORMBUY'];
		//spec_arr = getSelectedAttributes(formBuy);
		//Ajax.call('goods.php?act=get_products_info', 'id=' + spec_arr+ '&goods_id=' + goods_id, shows_number, 'GET', 'JSON');
		changePriceAll();
	...
	for(zzz=0; zzz<mylist.length; zzz++)
	{
		if(mylist[zzz].className!='wuxiao')
		{
				mylist[zzz].onclick=function(){
					show_attr_status(this, goods_id, attr_id_www_wygk_cn);
				}
		}
	...
	document.getElementById(the_input_id).checked="checked";
	 ...
	 {
	     selid_www_wygk_cn=0;
	     if (spec_attr_type[iii].value != www_wygk_cn)
	     {
		...
		{	
			...
			{					
				if (selid_www_wygk_cn)
				{
					if (s1_list[jjj].className == 'cattsel')
					{
						selid_www_wygk_cn = s1_a_id;
					}
				}
				else
				{
		...
		document.getElementById('spec_value_' + selid_www_wygk_cn).checked = "checked";
		selected_first.push(selid_www_wygk_cn);
		document.getElementById('xuan_a_'+selid_www_wygk_cn).className='cattsel';
		//document.getElementById('xuan_a_'+selid_www_wygk_cn).onclick=function(){}		
	     }
	}
	changeAtt(theid, goods_id);
	if (theid.parentNode.id.replace('catt_','')==attr_id_www_wygk_cn)
	{
 ...
 function is_exist_prod(selected_first, id, prod_exist_arr)
 {
	...
	var www_wygk_cn_selected = selected_first.slice(0);
	www_wygk_cn_selected.push(id);	
	var all_valid =0;
	for (var i in prod_exist_arr)
	{
		var first_exist=1;
		for (var j in www_wygk_cn_selected)
		{
			if ( prod_exist_arr[i].indexOf("|" + www_wygk_cn_selected[j] + "|") =='-1')
			{
				first_exist=0;
				break;
			}
